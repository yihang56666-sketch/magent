...
| 能力 | 原生子智能体工具（`create_thread` 等） | 本 Skill 增加的编排层 |
| --- | --- | --- |
...
本 Skill 刻意不做执行层。它依赖 Codex 原生子智能体能力（以实际会话 schema 暴露的线程工具为准）作为运行时，把自己的范围收敛到分派判断、角色边界、工作区保护、结果审计和失败恢复。它和通用框架不在同一层，更接近“编排契约/操作手册”与“执行引擎”的关系。

| 维度 | 通用多智能体框架 | 本 Skill |
| --- | --- | --- |
| 定位 | 执行引擎 / 运行时 | Codex 原生子智能体上的编排规则层 |
| 运行依赖 | 自带 agent 循环、工具与模型对接 | 依赖 Codex 原生子智能体工具 |
...
完成但未关闭的代理可能仍占并发额度。收集结果后使用当前会话的关闭/归档工具；例如暴露 `set_thread_archived` 时显式调用。该工具若返回关闭请求前的状态，不能将其误当作关闭后的状态。所有工具名和参数以实际会话 schema 为准，不假设固定的角色参数、模型名单或硬预算参数。
...
如果只需要一个聚焦复核，就只分派一个专家。主智能体收集报告、按 ID 关闭或归档不再需要的代理，解决冲突并验证证据后，才编辑实现和测试。若当前会话未暴露原生子智能体工具，则直接在本地调查，不模拟分派。
...
需要一个支持原生子智能体（`native subagent support`）的 Codex 会话，并且会话 schema 实际暴露原生子智能体工具（如 `create_thread`）。如果当前平台不提供该能力，Skill 会要求主智能体继续本地执行，而不是用复制提示词的方式伪造子智能体。
...
**当前会话没有原生子智能体工具能用吗？** 可以。它会退化为本地工作流，不会创建虚假的子智能体。
